import { GAME_W, GAME_H, RENDER, FPS, PHYSICS, TEX, GOB_SPAWN_EVERY_MS, GOB_AI_TICK_MS } from './config.js';
...
export class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // Разовые заготовки текстур
    const g = this.add.graphics();
    g.fillStyle(0x64b5ff, 1).fillCircle(6,6,6); g.lineStyle(2,0x9cd0ff,1).strokeCircle(6,6,6); g.generateTexture(TEX.bullet, 12,12); g.clear();
    g.fillStyle(0xf2cc60, 1).fillCircle(6,6,6); g.lineStyle(2,0xb89634,1).strokeCircle(6,6,6); g.generateTexture(TEX.xp, 12,12); g.clear();
    g.fillStyle(0xff6b6b, 1).fillCircle(10,10,10); g.lineStyle(2,0xff9a9a,1).strokeCircle(10,10,10); g.generateTexture(TEX.meteor, 20,20);
    g.destroy();
  }
...
// Фабрика конфига (чтобы не тянуть Phaser здесь)
